...
describe('IabTcf', () => {
    ...
    it('pingV1 handles error when __cmp throws', () => {
        (window as any).__cmp = vi.fn(() => { throw new Error('fail'); });
        const cb = vi.fn();
        iabTcf.pingV1(cb);
        expect(cb).toHaveBeenCalledWith(false);
    });
    ...
    it('pingV2 handles error when __tcfapi throws', () => {
        (window as any).__tcfapi = vi.fn(() => { throw new Error('fail'); });
        const cb = vi.fn();
        iabTcf.pingV2(cb);
        expect(cb).toHaveBeenCalledWith(null);
    });
    ...
    it('sendDetailsToContentScript skips emitting when message is duplicate', () => {
        ...
        // Test with v1 only to verify deduplication
        iabTcf.sendDetailsToContentScript();
        expect(EventBus.emit).toHaveBeenCalledTimes(1);

        // Call again with same data
        iabTcf.sendDetailsToContentScript();
        expect(EventBus.emit).toHaveBeenCalledTimes(1); // Second call skipped
    });

    it('sendDetailsToContentScript handles errors gracefully when pingV1 or pingV2 throw', () => {
        iabTcf.lastMessage = undefined;

        (window as any).__cmp = vi.fn(() => { throw new Error('cmp fail'); });
        (window as any).__tcfapi = vi.fn(() => { throw new Error('tcfapi fail'); });
