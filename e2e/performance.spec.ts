/**
 * Automated Performance Benchmark Suite for Professor Prebid
 * 
 * Measures real-world performance impact (Core Web Vitals, CDP Main Thread CPU,
 * Memory Heap, TBT / Long Tasks) on real-life websites with and without Professor Prebid.
 */
...
// Configurable target test sites (can be overridden via PERF_URLS env var)
const DEFAULT_SITES = [
    {
        name: 'eatpicks.com',
...
const SETTLE_TIME_MS = 5000; // Time to allow Prebid auctions and ads to settle
...
/**
 * Attempts to auto-accept common GDPR / CMP consent dialogs.
 */
...
/**
 * Runs a single measurement session for a URL.
 */
 ...
 */
function aggregateRuns(runs: PerformanceRunResult[]): PerformanceRunResult {
    return {
...
function calcDelta(base: number, ext: number) {
    const val = ext - base;
...
test.describe('Real-World Performance Benchmark', () => {
    test('Measures A/B performance on live publisher sites', async () => {
        ...
        const targetSites = customUrls
            ? customUrls.map(u => ({ name: new URL(u).hostname, url: u, description: 'Custom test URL' }))
            : DEFAULT_SITES;
        ...
        for (const site of targetSites) {
            console.log(`\n>>> Benchmarking Site: ${site.name} (${site.url})`);

            // 1. Baseline Runs (No Extension)
            console.log(`  [1/2] Running Baseline tests (without extension)...`);
            ...
            for (let i = 1; i <= ITERATIONS; i++) {
                ...
                console.log(`Done (LCP: ${res.lcp}ms, Script: ${res.scriptDurationMs}ms, Heap: ${res.jsHeapUsedMB}MB)`);
            }
            const baselineMedian = aggregateRuns(baselineRuns);

            // 2. Extension Runs (With Professor Prebid)
            console.log(`  [2/2] Running Extension tests (with Professor Prebid)...`);
        ...
        for (const s of summaries) {
            ...
            mdReport += `| **Long Tasks Count** | ${s.baseline.longTasksCount} | ${s.withExtension.longTasksCount} | ${s.delta.longTasksCount.val > 0 ? '+' : ''}${s.delta.longTasksCount.val} | ${s.delta.longTasksCount.pct > 0 ? '+' : ''}${s.delta.longTasksCount.pct}% |\n`;
            mdReport += `| **Main Thread Scripting** | ${s.baseline.scriptDurationMs} ms | ${s.withExtension.scriptDurationMs} ms | ${s.delta.scriptDurationMs.val > 0 ? '+' : ''}${s.delta.scriptDurationMs.val} ms | ${s.delta.scriptDurationMs.pct > 0 ? '+' : ''}${s.delta.scriptDurationMs.pct}% |\n`;
            mdReport += `| **JS Heap Used** | ${s.baseline.jsHeapUsedMB} MB | ${s.withExtension.jsHeapUsedMB} MB | ${s.delta.jsHeapUsedMB.val > 0 ? '+' : ''}${s.delta.jsHeapUsedMB.val} MB | ${s.delta.jsHeapUsedMB.pct > 0 ? '+' : ''}${s.delta.jsHeapUsedMB.pct}% |\n\n`;
        }
