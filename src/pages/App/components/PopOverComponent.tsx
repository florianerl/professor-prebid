  ...
  return (
    <Grid size={{ xs: 12, sm: 8, md: expanded ? 12 : 6 }} sx={{ minHeight: 250, height: expanded ? 'unset' : 250, overflow: 'hidden' }}>
      <Box sx={{ height: 1 }}>
        ...
        {children && children}
        {json && <JSONViewerComponent src={json} name={false} displayObjectSize={true} displayDataTypes={false} sortKeys={false} quotesOnKeys={false} indentWidth={2} collapsed={false} collapseStringsAfterLength={expanded ? undefined : 30} />}
      </Box>
    </Grid>
  );
};
