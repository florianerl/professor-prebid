...
test.describe('01 - Live Prebid Detection & Extension Lifecycle', () => {
  ...
  test('detects Prebid.js and TCF v2 CMP on German tech publisher (heise.de)', async () => {
    ...
    console.log(`[heise.de] Prebid Version: ${snapshot?.prebidVersion}, Installed Modules: ${snapshot?.installedModules?.length}`);
  ...
  test('detects Prebid.js and GAM slots on lifestyle site (eatpicks.com)', async () => {
    ...
    const snapshot = await liveSite.getMcpSnapshot();
    expect(snapshot).toBeDefined();
    console.log(`[eatpicks.com] Prebid Version: ${snapshot?.prebidVersion}, GAM Slots: ${Object.keys(snapshot?.gamTargeting || {}).length}`);
  ...
  test('detects Prebid.js and custom wrapper on news publisher (theguardian.com)', async () => {
    ...
    const snapshot = await liveSite.getMcpSnapshot();
    expect(snapshot).toBeDefined();
    console.log(`[theguardian.com] Prebid Version: ${snapshot?.prebidVersion}, Installed Modules: ${snapshot?.installedModules?.length}`);
