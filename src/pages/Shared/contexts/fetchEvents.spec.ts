...
describe('fetchEvents', () => {
    ...
    it('skips download if url is already in downloadingUrls array', async () => {
        ...
        // Note: 'in' operator checks indices/keys on array
        ...
        const tabInfo: any = {
            'top-window': {
                prebids: {
                    ...
                    workingPbjs: {
                        eventsUrl: 'https://otherdomain.com/working.json', // skipped because host mismatch
                    },
                },
            },
        };
    ...
    it('handles invalid URLs safely in safelyConstructURL and getCurrentTabURL', async () => {
        global.chrome = {
            tabs: {
                query: vi.fn((queryInfo, cb) => cb([{}])), // activeTab has no url property
            },
            windows: {
                WINDOW_ID_CURRENT: 1,
            },
